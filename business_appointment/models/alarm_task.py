...
class alarm_task(models.Model):
    ...
    def action_get_next_popup_notif(self):
        ...
        Here we calculate alarms for the next 24 hours only. The newest would be added by bus.
    ...
    def _do_popup_reminder(self):
        ...
        for task in self:
            partners, channels, internal = task._get_recipients()  
            ...
            if internal and partner_id.id in partners.ids:
                ...
                res.append({
                    ...
                    "message": task.appointment_id.return_scheduled_time_tz(),
                    "resource": task.appointment_id.resource_id.name,
                    "service": task.appointment_id.service_id.name,
                    "partner": task.appointment_id.partner_id.name,
                    "timer": delta,
    ...
    def _do_email_reminder(self):
        ...
        partners, channels, internal = self._get_recipients()      
        ...
        for lang, partner_recordset in partners_by_languages.items():
            ...
            channels_lang = lang == default_lang and channels.ids or []
    ...
    def _get_recipients(self):
        ...
        channel_ids = self.env["mail.channel"]
        ...
        if recipients == "user_id":
            ...
            if recipients == "everybody":
                channel_ids = appointment_id.message_channel_ids
            else:
        ...
        return partner_ids, channel_ids, internal
    ...
    def _nofity_popup_bus(self, partner_ids):
        ...
        for user in users:
            if user.has_group("business_appointment.group_ba_user"):
                ...
                notifications.append([(self._cr.dbname, 'alarm.task', user.partner_id.id), popups])
