...
class business_appointment(models.Model):
    ...
    def _remove_outdated_alarms(self):
        ...
        for bus_alarm in bus_alarms:
            partners, channels, internal = bus_alarm._get_recipients()
        ...
        if not self.return_not_topical():
            ...
            for alarm in self.alarm_ids:
                ...
                if not alarm_time or alarm_time < now:
                    ...
                    partners, channels, internal = task_id._get_recipients()
                    bus_partners += partners
            if bus_partners:
                self.env["alarm.task"]._nofity_popup_bus(bus_partners.ids)


    def action_open_form_view(self, alarm_id):
        """
        ...
        """
        action_id = False
        if self and alarm_id:
            task_id = self.env["alarm.task"].browse(alarm_id)
            if task_id.exists():
                task_id.notified_partner_ids
                task_id.action_mark_popup_done()
            action_id = self.sudo().env.ref("business_appointment.business_appointment_action_only_form").read()[0]
