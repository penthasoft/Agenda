...
class SitradVictimas(models.Model):
    _name = "sitrad.victimas"
    # _inherit = ["mail.thread", "mail.activity.mixin"]
    _description = "Victimas"

    def _victima_age(self):
    ...
    age = fields.Char(compute=_victima_age, size=32, string='Edad', readonly=False, store=True, index=True)
    age_year = fields.Char(compute=_victima_age_year, size=32, string='Edad Años', readonly=False, store=True, index=True)
    age_month = fields.Char(compute=_victima_age_month, size=32, string='Edad Meses', readonly=False, store=True, index=True)
    ...
    condicion = fields.Many2one('sitrad.condicion', string='Condicion', store=True, index=True)
    ...
    def create(self, vals):
        sequence = self.env['ir.sequence'].next_by_code('sitrad.victimas')
        vals['identificador'] = sequence or '/'
        inno_victimas = super(SitradVictimas, self).create(vals)
        return inno_victimas
    ...
    def _onchange_nro_doc(self):
        ...
        validar_obj = self.env['sitrad.victimas']
...
class SitradDenVict(models.Model):
    _name = 'sitrad.den_victima'
    ...
    victima = fields.Many2one('sitrad.victimas', string="Victima Den.")
    denuncia_id = fields.Many2one('sitrad.denuncias', string="Nro. Denuncia")

    @api.model
    def create(self, vals):
        sequence = self.env['ir.sequence'].next_by_code('sitrad.den_victima')
        vals['name'] = sequence or '/'
        inno_victimas = super(SitradDenVict, self).create(vals)
        return inno_victimas
