    ...
    def send_whatsapp_automatic(self):
        for task in self:
            ...
            if self._get_whatsapp_server() and self._get_whatsapp_server().status == 'authenticated':
                ...
                try:
                    body = body.replace('_PARTNER_', task.partner_id.name)
                except:
                ...
                for partner in partners:
                    if partner.country_id and partner.whatsapp:
                        ...
                        message_data = {
                            'phone': whatsapp,
                            'body': html2text.html2text(body) + task.get_link(),
                        }                        
                        if partner.chat_id:
                ...
                vals = WhatsappComposeMessage._prepare_mail_message(self.env.user.partner_id.id, AllchatIDs, task and task.id,  'project.task', body, message_data, subject, partners.ids, attachment_ids, send_message, status)
